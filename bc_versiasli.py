...
def create_spend_chart(charts):
    ...
    for i in range(len(percentage)):
        ...
        for j in range(0,len(graph_height)):
          if j==0:
            ...
            graph_position[i].append((graph_height[j][i]).ljust(3,"*"))
    print(graph_position)

    nl='\n'
    graph=f"{nl.join((str(''.join(str(y) for y in x))) for x in graph_position)}"
    ...
    for i in range(len(left_empty)):
        legend_list.append([left_empty[i].rjust(5)])
        for j in range(0,len(category_legend)):
            legend_list[i].append((category_legend[j][i]).ljust(3,"*"))

    nl='\n'
    legend=f"{nl.join((str(''.join(str(y) for y in x))) for x in legend_list)}"
